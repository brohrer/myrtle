...
def report_buckettree():
    ...
    for dirname in os.listdir(os.path.join(log_directory, "buckettree")):
        if dirname[:5] == "tree_":
            n_trees += 1
            highs.append(np.load(
                os.path.join(log_directory, "buckettree", dirname, "highs.npy")
            ))
            lows.append(np.load(
                os.path.join(log_directory, "buckettree", dirname, "lows.npy")
            ))
            levels.append(np.load(
                os.path.join(log_directory, "buckettree", dirname, "levels.npy")
            ))

    fig, axes_list = blank_images(n_trees)

    for i_ax in range(n_trees):
        ax = axes_list[-i_ax - 1]
        draw_buckets(ax, highs[i_ax], lows[i_ax], levels[i_ax])
        # print(bucket_info_list)
        # left_title(ax, f"sensor {i_ax}")
        ax.set_xlabel(
            f"sensor {i_ax}",
            color=color,
